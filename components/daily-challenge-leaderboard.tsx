...
export function DailyChallengeLeaderboard({ className, date }: DailyChallengeLeaderboardProps) {
  ...
  useEffect(() => {
    async function fetchLeaderboard() {
      try {
        ...
        let url = `/api/leaderboard/daily?date=${displayDate}`
        if (username) {
    ...
    <Card className={className}>
      <CardHeader className="pb-3">
        <CardTitle className="flex items-center gap-2 text-lg">
          ...
          <span>Daily Challenge: {formattedDate}</span>
        </CardTitle>
      </CardHeader>
      <CardContent>
        {loading ? (
          ...
          <>
            <div className="space-y-2">
              {leaderboard.length > 0 ? (
                ...
                <p className="text-center py-4 text-muted-foreground">No players yet for this daily challenge!</p>
              )}
            </div>
